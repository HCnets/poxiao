...
internal fun PoxiaoOverlayHost(
    ...
    with(scaffoldState) {
        AnimatedContent(
            targetState = overlayPage,
            transitionSpec = {
                val isEntering = targetState != null
                if (isEntering) {
                    (slideInVertically { fullHeight -> fullHeight } + fadeIn(tween(320)))
                        .togetherWith(slideOutVertically { fullHeight -> -fullHeight / 3 } + fadeOut(tween(220)))
                } else {
                    (slideInVertically { fullHeight -> -fullHeight / 3 } + fadeIn(tween(220)))
                        .togetherWith(slideOutVertically { fullHeight -> fullHeight } + fadeOut(tween(320)))
                }
            },
