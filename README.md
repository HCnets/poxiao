...
| `当前` | **v1.5.3.2** | **手势 API 兼容性补丁 (Gesture API Patch)** | 修复了 ScientificCalculator.kt 中的 `awaitEachGesture` 引用错误与 `PointerInputChange` 成员访问冲突。 |
| `历史版本` | **v1.5.3.1** | **手势反馈紧急修复 (Gesture & Haptic Patch)** | 重构 KeypadButton 手势系统，解决侧滑触发困难与气泡闪烁问题；增强 Haptic 震动强度，确保物理反馈真实可感知。 |
