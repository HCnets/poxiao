...
| `当前` | **v1.5.1.1** | **编译稳定性补丁 (Compilation Stability Patch)** | 修复了 ScientificCalculator.kt 中的作用域引用错误 (statsRawX/Y)、CalcButton 参数识别异常以及 Int 比较时的操作符修饰符误报问题。 |
| `历史版本` | **v1.5.1** | **液态玻璃 2.0 深度优化 (Refined Glassmorphism)** | 进一步细化了 Liquid Glass 2.0 的渲染细节，包括卡片边框的呼吸灯效、头部区域的动态发光以及更流畅的按键反馈动画。 |
