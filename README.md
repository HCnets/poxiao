...
| `13` | **v1.2.0** | **多智能体与液态玻璃 2.0** | 落地基于协程并行的专家智能体系统 (MAS)，UI 引入物理视差陀螺仪动效与风格无级自定义。 |
| `14` | **v1.2.1** | **骨架屏与修复** | 全面引入骨架屏 (SkeletonPlaceholder) 过渡状态，修复多处编译时重声明与模型冲突。 |
| `15` | **v1.2.2** | **陀螺仪调优** | 为陀螺仪视差引入 Leaky Integrator 自居中算法，解决初始姿态偏移与抖动问题。 |
| `16` | **v1.2.3** | **全局组件大一统** | 将二级页面老旧 Surface 替换为 LiquidGlass，引入顶层 Z 轴悬浮岛 (Floating Header) 布局。 |
| `17` | **v1.2.4** | **人机工程学重构** | 对重度输入模块进行视线焦点与按键网格重组，支持不等宽键盘与实时公式推演反馈。 |
| `18` | **v1.2.5** | **便当盒布局 (Bento)** | 彻底重构首页模块加载引擎，全面引入 Bento Box (便当盒) 交错网格布局，提升空间利用率。 |
| `19` | **v1.2.6** | **黄金流交互革命** | 彻底重构科学计算器，采用固定底部键盘布局，引入 `FocusTarget` 全局焦点路由系统。 |
| `20` | **v1.2.7** | **黑曜石霓虹进化** | 全量落地计算器双主题美学 (iOS-like / Obsidian)，为所有交互组件注入物理级触觉反馈。 |
| `21` | **v1.2.8** | **专业工具链与手势** | 重构方程、复数、向量等专业模块，支持极坐标运算；为历史记录引入侧滑删除高阶交互。 |
| `22` | **v1.2.9** | **工作区头部修复** | 优化顶部信息卡片排版，将标题由纵向改为横向紧凑排列，解决面板遮挡问题。 |
| `23` | **v1.3.0** | **终极计算器** | 引入 Swipe-on-Keys 全域侧滑交互、Undo/Redo 堆栈、Base-N 实时预览与智能括号补全。 |
| `24` | **v1.3.1** | **自然数学引擎** | 实现自然数学渲染 (WYSIWYG)，支持分式、根号递归排版，开发物理光标逻辑跳转系统。 |
| `25` | **v1.3.2** | **感知增强** | 引入实时函数图像预览 (Sparkline)，利用物理引擎实装光标刻度感触觉。 |
| `26` | **v1.3.3** | **单位感知计算** | 科学计算器正式支持物理单位感知计算，实现 `UnitDimension` 量纲代数系统。 |
| `27` | **v1.3.5** | **架构稳定性加固** | 实施递归深度监控 (上限 100 层) 与表达式长度限制，注入全局异常拦截器。 |
| `28` | **v1.3.6** | **渲染管线修复** | 修正 Compose 渲染管线中的非法 Composable 调用，下沉 UI 防御逻辑至解析层。 |
| `29` | **v1.3.7** | **历史记录稳定性** | 引入 `HistoryRecord` 唯一标识符系统，解决长列表在反向布局下的键值冲突闪退。 |
| `30` | **v1.3.8** | **可视化网格输入** | 重构矩阵与向量模块为可视化网格布局，支持物理单元格导航与嵌套渲染，实现类 Excel 跳转。 |
| `当前` | **v1.3.8.3** | **架构状态补丁** | 修复模块容器中的 `val` 重赋值编译错误，统一状态提升 (State Hoisting) 链路。 |
